...
final class OfflineFirstReservationRepository {
   ...
   // private let reservationRepo = ReservationRepository.shared
    ...
    func getOfflineReservations(isPast: Bool, isCanceled: Bool) async -> [Reservation] {
        let result = await loadOnlineReservations(isPast: isPast, isCanceled: isCanceled)
        if (result.isSuccess){
            return result.data!
        }
        return []
...
//        }
    ...
    func loadOnlineReservations(isPast: Bool, isCanceled: Bool) async -> Result<[Reservation]> {
...
//        }))
        ...
        return .success(data: reservations.map({ ReservationDto in
            Reservation(fromDto: ReservationDto)
        }))
    }
